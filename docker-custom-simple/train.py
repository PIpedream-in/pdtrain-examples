...
logger = logging.getLogger("docker-custom-simple")
...
def load_or_synthesize(data_dir: str) -> Tuple[np.ndarray, np.ndarray, list]:
...
def main():
    ...
    p.add_argument("--fit_intercept", type=str, default="true")
    p.add_argument("--normalize", type=str, default="false")
    ...
    X, y, cols = load_or_synthesize(data_dir)
    model = LinearRegression(fit_intercept=str2bool(args.fit_intercept))
    model.fit(X, y)
    ...
    (out / "metrics.json").write_text(json.dumps({
        "mse": mse,
        "fit_intercept": str2bool(args.fit_intercept),
